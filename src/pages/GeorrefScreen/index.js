...
class GeorrefScreen extends Component {
  ...
  fitAllMarkers() {
    this.props.casos.casos && this.map.fitToCoordinates(
      this.props.casos.casos.map((c) => c.geolocalizacao),
      {
  ...
  componentDidMount() {
    this.props.casos.casos && this.fitAllMarkers();
  }
    ...
    return (
      <View style={{ flex: 1 }}>
        <MapView
          ...
          loadingEnabled={true}
        >
          {this.props.casos.casos && this.props.casos.casos.map((marker, i) => (
            <Marker
              key={i}
              identifier={`id${i}`}
              coordinate={marker.geolocalizacao}
              title={marker.dados_pessoais.nome}
              description={marker.dados_residenciais.endereco}
              pinColor={
                marker.dados_conclusao.caso_confirmado ? 'red' : 'orange'
              }
            />
          ))}
        </MapView>
          ...
          <View style={estilo.itemContainer}>
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  onPress={() => {
                    this.fitOneMarker(item.geolocalizacao);
                  }}
                >
                  <Text style={estilo.item}>{item.dados_pessoais.nome}</Text>
                </TouchableOpacity>
              )}
